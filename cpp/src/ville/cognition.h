...
// plans, talks, or reflects; a Cognition decides *what* -- the same split the
// paper's code has between its reverie loop and its GPT prompt templates.
//
...
//   LlmCognition -- the paper's setup: prompts to the agent's own model
...
// The paper's 25 residents of the Ville (personas paraphrased from the
// released simulation), then generated residents for larger towns.
std::vector<Persona> makePersonas(const World& world, int population, uint32_t seed);

// Seed knowledge each run starts with (Isabella's party, Sam's campaign).
